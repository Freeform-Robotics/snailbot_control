...
float BMI088_temperature_read_over(const uint8_t *rx_buf) {
    ...
    return (float)bmi088_raw_temp * BMI088_TEMP_FACTOR + BMI088_TEMP_OFFSET;
}

void BMI088_accel_read_over(const uint8_t *rx_buf, float accel[3]) {
    int16_t bmi088_raw_temp;
...
void BMI088_gyro_read_over(const uint8_t *rx_buf, float gyro[3]) {
    int16_t bmi088_raw_temp;
    bmi088_raw_temp = (int16_t)((rx_buf[1]) << 8) | rx_buf[0];
...
void BMI088_read(float gyro[3], float accel[3], float *temperature) {
    ...
    *temperature = get_BMI088_temperature();
}
...
extern void get_BMI088_temperature_raw(uint8_t temp[2]) {
    BMI088_accel_read_muli_reg(BMI088_TEMP_M, temp, 2);
}

float get_BMI088_temperature(void) {
    uint8_t buf[2];
    get_BMI088_temperature_raw(buf);
...
void BMI088_set_accel_int_drdy(int pin) {
    if (pin == 1) {
        BMI088_accel_write_single_reg(BMI088_INT_MAP_DATA, BMI088_ACC_INT1_DRDY_INTERRUPT);
        BMI088_accel_write_single_reg(BMI088_INT1_IO_CTRL, BMI088_ACC_INT1_IO_ENABLE | BMI088_ACC_INT1_GPIO_PP | BMI088_ACC_INT1_GPIO_HIGH);
    } else if (pin == 2) {
        BMI088_accel_write_single_reg(BMI088_INT_MAP_DATA, BMI088_ACC_INT2_DRDY_INTERRUPT);
        BMI088_accel_write_single_reg(BMI088_INT2_IO_CTRL, BMI088_ACC_INT2_IO_ENABLE | BMI088_ACC_INT2_GPIO_PP | BMI088_ACC_INT2_GPIO_HIGH);
    }
}

void BMI088_set_gyro_int_drdy(int pin) {
    if (pin == 3) {
        BMI088_gyro_write_single_reg(BMI088_GYRO_INT3_INT4_IO_MAP, BMI088_GYRO_DRDY_IO_INT3);
        BMI088_gyro_write_single_reg(BMI088_GYRO_INT3_INT4_IO_CONF, BMI088_GYRO_INT3_GPIO_PP | BMI088_GYRO_INT3_GPIO_HIGH);
    } else if (pin == 4) {
        BMI088_gyro_write_single_reg(BMI088_GYRO_INT3_INT4_IO_MAP, BMI088_GYRO_DRDY_IO_INT4);
        BMI088_gyro_write_single_reg(BMI088_GYRO_INT3_INT4_IO_CONF, BMI088_GYRO_INT4_GPIO_PP | BMI088_GYRO_INT4_GPIO_HIGH);
    }
    BMI088_gyro_write_single_reg(BMI088_GYRO_CTRL, BMI088_DRDY_ON);
}
